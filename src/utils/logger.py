...
class ExperimentLogger:
    ...
    PER_TAG_HEADER = (
        "| Model | Tag | Precision | Recall | F1 | Support | Predicted |\n"
    )
    PER_TAG_DIVIDER = "|-------|-----|-----------|--------|----|---------|----------|\n"
    ...
    def _ensure_sections(self):
        # If file exists but is missing either section, prefer to attach the section
        # header above any existing table that matches the expected header row.
        with open(self.log_path, "r", encoding="utf-8") as f:
            lines = f.readlines()

        changed = False

        # Helper to ensure a section header exists above a found table header line
        def ensure_section_above(header_row, section_title):
            nonlocal lines, changed
            for idx, line in enumerate(lines):
                if line.strip() == header_row.strip():
                    # Check if the section title already exists within a few lines above
                    lookback = max(0, idx - 4)
                    found = any(
                        lines[i].strip() == section_title for i in range(lookback, idx)
                    )
                    if not found:
                        lines.insert(idx, "\n")
        ...
        # If neither table exists, append both sections at the end (fresh file case)
        if not global_found and not per_tag_found:
            lines.append("\n")
            lines.append(f"{self.GLOBAL_SECTION}\n\n")
    ...
    def _insert_row_in_table(self, table_header_start: str, row: str):
        # Read file and insert a row into the table whose header line starts with table_header_start
        with open(self.log_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        # find the header line index (match exact header row or starting token)
        header_idx = None
        for i, line in enumerate(lines):
            if line.strip().startswith(table_header_start) or line.strip() == table_header_start.strip():
                header_idx = i
                break

        if header_idx is None:
            # fallback: try locating the full known headers (useful if caller passed a short token)
            for i, line in enumerate(lines):
        ...
        if header_idx is None:
            # still not found: append at end with a preceding blank line for readability
            with open(self.log_path, "a", encoding="utf-8") as f:
                f.write("\n")
                f.write(row)
            return

        # Move forward from header_idx to find the last consecutive table line (lines that start with '|')
        j = header_idx
        while j + 1 < len(lines) and lines[j + 1].lstrip().startswith("|"):
            j += 1
        insert_at = j + 1

        # Insert the row at insert_at
    ...
    def log_result(self, model_name, embedding, classifier, dataset, metrics, notes=""):
        ...
        metrics should include keys: precision_macro, recall_macro, f1_macro, hamming_loss
        and optionally 'per_tag' which is a list of dicts with keys: tag, precision, recall, f1, support, predicted
        """
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")

        global_line = (
            f"| {timestamp} | {model_name} | {embedding} | {classifier} | {dataset} | "
            f"{metrics.get('precision_macro', 0):.4f} | {metrics.get('recall_macro', 0):.4f} | {metrics.get('f1_macro', 0):.4f} | "
            f"{metrics.get('hamming_loss', 0):.4f} | {notes} |\n"
        )

        # Insert into global table (table header starts with '| Date |')
        ...
        if per_tag:
            for tag_metrics in per_tag:
                tag_line = (
                    f"| {model_name} | {tag_metrics.get('tag', '')} | "
                    f"{tag_metrics.get('precision', 0):.4f} | {tag_metrics.get('recall', 0):.4f} | {tag_metrics.get('f1', 0):.4f} | "
                    f"{tag_metrics.get('support', 0)} | {tag_metrics.get('predicted', 0)} |\n"
                )
                # Insert into per-tag table (table header starts with '| Model |')
